...
server <- function(input, output) {
    ...
    load('D:\\James\\Documents\\tamu\\STAT656\\Homework\\project\\code\\STAT656_Project\\Li\\sp500-4year.RData')
    ...
    output$elbowPlot <- renderPlot({
        ...
        plotW = rep(0,9)
        for(K in 2:10){
            plotW[K] = kmeans(clust@distmat,centers=K,nstart=25)$tot.withinss
        }
        plot(plotW)
    })
