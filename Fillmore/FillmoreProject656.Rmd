...
if (file.exists("SP500-3year.RData")) {
  load("SP500-3year.RData")
} else {
  returns <- tq_index('SP500') %>%
    ...
    drop_na
  save(returns,file="SP500-3year.RData")
}
```
...
if (file.exists("SP500-3year-vol.RData")) {
  load("SP500-3year-vol.RData")
} else {
  returns_vol <- returns %>%
  ...
  save(returns_vol,file="SP500-3year-vol.RData")
}
```
